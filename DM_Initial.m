...
%           mol frac
    ...
    for j = 2:DM_stage
        ...
        if j<50
            Dual_Profile_Initial(:,j) = Dual_Initial_Stage_Cal0(stage_feed);  %NEW:
        else
...
function f = Dual_Initial_Stage_Cal0(stage_feed)     %后面加个0，专门给透量为零那段膜用
...
%           mol frac
    ...
    xrj0 = stage_feed(4:end);
    yi_DM1 = y_cross(xrj0,Ji_DM0,Pf_DM,Pp_DM1*2);
    yi_DM2 = y_cross(xrj0,Ji_DM2,Pf_DM,Pp_DM2*2);
    y1 = sum(yi_DM1);
    y2 = sum(yi_DM2);
    ...
    Qi_DM1 = zeros(c,1);Qi_DM2 = zeros(c,1);
    for i = 1:c
        Qi_DM1(i) = Ji_DM0(i)/3/829561.4*dA_DM1*(Pf_DM*xrj0(i)-Pp_DM1*yi_DM1(i));    %re:传质方程 Ji是渗透的那个膜透量参数（GPU单位)
        Qi_DM2(i) = Ji_DM2(i)/3/829561.4*dA_DM2*(Pf_DM*xrj0(i)-Pp_DM2*yi_DM2(i));
    end
    Q_DM1 = sum(Qi_DM1);
    Q_DM2 = sum(Qi_DM2);
